...
300 participants between the ages of 12 - 40 years old were randomly assigned to to one of the three subject groups CBT, VR Therapy and Control. The individuals underwent 12 weeks of their respective treatment for 50 minutes per week alongside a therapist.
...
Descriptive statistics, hypothesis testing and statistical revealed significant improvements in both therapy groups compared to the control.
...
4. The Change_Score was calculated as: Change_Score = P1_Pre_Score - P2_Post_Score. For this study, a positive change score indicates improvement (reduction in the severity autistic traits).
...
Descriptive analysis in Table 1 and mean change score by group bar chart Figure 1, reveals:

* VR: Highest mean improvement (M=19.43, SD=5.58), with scores ranging 8 - 33.
* CBT: Moderate improvement (M=10.16, SD=4.51), with scores ranging 0 - 22.
* Control: Negligible change (M=0.01, SD=4.45), with scores ranging -11 - 10.
...
```
Figure 2 boxplot illustrates the median of VR~19, CBT~10, Control~0 and no extreme outliers (beyond 1.5 times the interquartile range) are observed.
...
```
CBT and VR groups are normally distributed p = 0.3543 and p = 0.3092 > 0.05 respectively. The Control group p = 0.0188 and > 0.05 suggesting deviation from normality.
...
* Slight deviation from normality in the control group (p=0.0188). A better group divide would be more appropriate.
